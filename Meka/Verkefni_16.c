...
void main() {
	...
	while (1) {
		acd_res = ADC_Read(2); // Notum analog inngang 2 fyrir þett
		if (adc_res >= 550 && adc_res <= 450) {
			PORTC = 0x00;
			Lcd_out(2, 1, "Stop"); //skrifar á LCD að motorinn sé stopp
		}
		else if (adc_res >= 600) { //skrifar á LCD að motorinn sé að snúast áfram
			forward();
			Lcd_out(2, 1, "Afram");
		}
		else if (adc_res <= 400) { //skrifar út á LCD að motorinn sé að bakka
			backward();
			Lcd_out(2, 1, "Bakka");
		}
	}
	void forward(unsigned int adc_res) {	// Hámarkshraði áfram er 10ms og lægsti hraði er 30ms
		PORTC = 0x09;
	...
	void backward(unsigned int adc_res) {	// Hámarkshraði aftruábak er 10ms og lægsti hraði er 30ms
		PORTC = 0x0C;
